...
* it is also good practice to refer to packages needed for code to run, in the place *where* that code is implemented
    + this is less important for commonly-used packages like `ggplot2` and (much) more important for less-used packages
...
* I have code below that allows you to:
    + make a list of all the packages you want / need for your Rmarkdown file
    + load the packages
* the code in the chunk below doesn't work for packages stored in non-CRAN repositories (e.g. GitHub)
...
* to import your data:
    ...
    + **NEVER EVER** use an absolute file path (e.g., `C:/Windows/Bobsdirectory/dataset1`) anywhere, but they are most common in data import code
...
* But that *doesn't* mean that those checks should be printed out in your report---they probably shouldn't, they will make the report feel cluttered
    + one thing I often do is to put in the code for my checks, but to comment it out (e.g., `# head(dat)` or `# str(iris)` or `# View(iris)`) so that it doesn't clutter up the report, but someone examining the code can see that I did go through and check it out
...
In this context, I think it is helpful to put code into the text section of the report (i.e., not in a chunk) so that it can be, well, contextualized. For example, typing \`r str(iris)\` returns basic information on the structure of that dataset:

`r str(iris)`

Sometimes it makes sense to include a code chunk here, if needed to make more complex calculations; anything more than a couple of operations is typically best left to a code chunk, and can then be reported on in the text below the code chunk. The chunk below includes code from Loy et al.'s analysis; the lines of codes included outside of the chunk are repeated within the chunk (but commented out and thus not evaluated in the chunk). While this is by definition repetitive, it makes it easy for a reader of the Rmarkdown report to see how the calculations were conducted, without having to separately open the .Rmd file.
...
* often you will want to save a plot, for example for submitting with a paper; but keep the code for saving commented out so that you don't repeatedly save new versions
    + the reasoning behind this is that Git / GitHub does well when versioning flat files like text files / .R / .Rmd / .csv files (etc.) and in particular only changes the differences
    + Git cannot do that with binary files like image files / .pdfs / Word & Excel docs; thus it is re-writing the *entire* file every time it gets updated, even with a tiny change
    + thus, it is best to save image files as few times as possible; thus keep the code commented out and manually run it only as needed
...
# SAVE PLOT
# =============================
# (keep the actual code commented out to prevent repeated saving on GitHub)
# re-save when this changes, but only as needed
ggsave(myplot, file = "plots/myplot.pdf", width = 5, height = 3)
# **IMPORTANT**: note the "plots/" prefix in the name, that saves your plot in
# the `plots` folder of your project. ALWAYS use that!
