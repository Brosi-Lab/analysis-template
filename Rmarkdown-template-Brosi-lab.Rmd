...
* typically I strive to having one overall "master" dataset and sometimes one dataset per major group of analyses
    + I almost always remove other components: model objects, plot objects, etc. etc. unless they are needed downstream in the analysis
...
* it is also good practice to refer to uncommon packages needed for code to run, in a comment in the place *where* that code is implemented (but not to load them there, just to comment that function X is from package Y)
    + this is not particularly important for commonly-used packages like `ggplot2` but is (much) more important for less-used packages
* a minor pet peeve of mine is the package startup messages that can massively clutter up Rmarkdown reports
    + I include `message = F, results = F, warning = F` in the code chunk header to suppress those messages...
    + ...but there can be important warnings or errors; they will appear in this window when the code chunk is run, though not in the .html report
    + the "best of both worlds" approach included here uses the `evaluate` package which allows for saving of messages arising from whatever process you specify (including package loading) and then printing them at the **end** of the report, along with `sessionInfo`; this keeps your report clean and streamlined, while preserving that information which can be useful for troubleshooting (though typically that is rare)
...
```{r packages, message=FALSE, warning=FALSE, results=F}
