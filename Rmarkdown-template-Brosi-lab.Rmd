...
while you should *eventually* delete this text from your report, I suggest that you keep it around while you are coding, as a reminder to keep things streamlined and clean. Once an analysis gets to the very final stages (and assuming you have a clean workspace at that point!) you can delete.
...
* one exception to this is that it can be (very) annoying to have to re-run everything above, say, a particular code chunk to get that code chunk to work again, if you delete some of the previous objects or objects you create in that chunk
    ...
    + but once it is working to my satisfaction, I will un-comment it to 
...
* do **NOT** print out a whole dataset into your report unless it is really tiny
...
Data formatting, cleaning, and munging is typically the most time-consuming part of any analysis. Much of the time it makes sense to divide this into multiple code chunks, don't hesitate to do so if it makes it logically easier to follow.
...
* often you will want to save a plot, for example for submitting with a paper; but keep the code for saving commented out so that you don't repeatedly save new versions
    ...
    + Git cannot do that with binary files like image files / .pdfs / Word & Excel docs
    + so it is best to save it as few times as possible; thus keep the code commented out and manually run it only as needed
